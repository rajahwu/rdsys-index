...
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'brand.primary', '#1C2C4B', '28, 44, 75', '95, 82, 50, 45', '534 C', 'Deep Indigo — primary brand identity, headings, key emphasis, trust and depth',  TRUE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'brand.secondary', '#A78A55', '167, 138, 85', '30, 40, 75, 20', '4505 C', 'Warm Gold — achievement, value, highlights, progress indicators',  TRUE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'brand.neutralDark', '#4A4A4A', '74, 74, 74', '60, 52, 52, 45', 'Cool Gray 11 C', 'Stone Grey — body text, secondary headings, grounded elements',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'brand.neutralLight', '#F5F5F5', '245, 245, 245', '3, 2, 2, 0', 'Cool Gray 1 C', 'Soft Background — page base, card surfaces, breathing space',  FALSE),

-- Additional neutrals    
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'neutral.white', '#FFFFFF', '255, 255, 255', '0, 0, 0, 0', 'White', 'Pure white — overlays, highest contrast',  FALSE),    
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'neutral.mid', '#9E9E9E', '158, 158, 158', '40, 32, 32, 5', 'Cool Gray 6 C', 'Mid grey — borders, dividers, inactive states',  FALSE),    
    
-- Semantic accent colors    
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'accent.success', '#88A57B', '136, 165, 123', '45, 20, 55, 5', '5635 C', 'Sage Green — success states, completion, mastery achieved',  FALSE),    
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'accent.warning', '#E2B13C', '226, 177, 60', '10, 30, 85, 0', '7548 C', 'Amber — warnings, attention needed, caution',  FALSE),    
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'accent.error', '#C5544D', '197, 84, 77', '20, 75, 70, 10', '7418 C', 'Terra Cotta — errors, critical alerts, needs correction',  FALSE);
...
VALUES  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'phases', 'genesis', 'icons/phases/genesis.svg', '24x24', '2px', 'Genesis phase — seed sprouting',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'phases', 'source-code', 'icons/phases/source-code.svg', '24x24', '2px', 'Source Code phase — foundation blocks',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'phases', 'powerhouse', 'icons/phases/powerhouse.svg', '24x24', '2px', 'Powerhouse phase — energy core',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'phases', 'excellence', 'icons/phases/excellence.svg', '24x24', '2px', 'Excellence phase — refined gem',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'phases', 'mastery', 'icons/phases/mastery.svg', '24x24', '2px', 'Mastery phase — summit peak',  TRUE);

-- Tool/Action Icons  
INSERT INTO brand_icons (brand_id, category, name, file_path, size, stroke_width, description, is_primary_glyph)  
VALUES  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'tools', 'practice', 'icons/tools/practice.svg', '24x24', '2px', 'Practice action — repetition symbol',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'tools', 'reflect', 'icons/tools/reflect.svg', '24x24', '2px', 'Reflect action — thought bubble',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'tools', 'progress', 'icons/tools/progress.svg', '24x24', '2px', 'Progress indicator — upward arrow',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'tools', 'complete', 'icons/tools/complete.svg', '24x24', '2px', 'Complete action — checkmark',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'tools', 'bookmark', 'icons/tools/bookmark.svg', '24x24', '2px', 'Bookmark — save for later',  FALSE);

-- Navigation Icons  
INSERT INTO brand_icons (brand_id, category, name, file_path, size, stroke_width, description, is_primary_glyph)  
VALUES  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'nav', 'home', 'icons/nav/home.svg', '24x24', '2px', 'Home navigation — house outline',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'nav', 'lessons', 'icons/nav/lessons.svg', '24x24', '2px', 'Lessons navigation — stacked pages',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'nav', 'progress-chart', 'icons/nav/progress-chart.svg', '24x24', '2px', 'Progress navigation — ascending chart',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'nav', 'resources', 'icons/nav/resources.svg', '24x24', '2px', 'Resources navigation — toolbox',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'nav', 'profile', 'icons/nav/profile.svg', '24x24', '2px', 'Profile navigation — user circle',  FALSE);

-- Status Icons  
INSERT INTO brand_icons (brand_id, category, name, file_path, size, stroke_width, description, is_primary_glyph)  
VALUES  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'status', 'in-progress', 'icons/status/in-progress.svg', '24x24', '2px', 'In progress — circular progress indicator',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'status', 'completed', 'icons/status/completed.svg', '24x24', '2px', 'Completed — filled circle with check',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'status', 'locked', 'icons/status/locked.svg', '24x24', '2px', 'Locked — padlock',  FALSE),  
((SELECT id FROM brands WHERE slug = 'vsm-school'), 'status', 'available', 'icons/status/available.svg', '24x24', '2px', 'Available — open door',  FALSE);
...
VSM School palette evokes elevation and calm mastery. Deep Indigo provides trustworthy depth. Warm Gold signals achievement. Natural, muted semantic tones fit the misty mountain aesthetic.',  TRUE),
