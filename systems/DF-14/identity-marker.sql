...
((SELECT id FROM brands WHERE slug = 'dropframe'), 'dropframe.teal', '#006E6D', '0, 110, 109', '100, 25, 30, 30', '3282 C', 'Cobbs Creek Teal — primary brand accent, calm energy, digital modernity', 1),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'dropframe.red', '#C41230', '196, 18, 48', '10, 95, 80, 10', '186 C', 'Colonial Red — calls to action, focus, urgency', 1),

-- Tier colors    
((SELECT id FROM brands WHERE slug = 'dropframe'), 'tier.gold', '#FFC857', '255, 200, 87', '0, 20, 70, 0', '1225 C', 'Tier 1 accent — Warm Gold, achievement highlights', 0),    
((SELECT id FROM brands WHERE slug = 'dropframe'), 'tier.lavender', '#B497BD', '180, 151, 189', '35, 35, 5, 0', '2567 C', 'Tier 2 accent — West Philly Lavender, trust, polish', 0),    
((SELECT id FROM brands WHERE slug = 'dropframe'), 'tier.slate', '#4B4B4B', '75, 75, 75', '60, 52, 52, 40', 'Cool Gray 11 C', 'Tier 3 accent — Slate Gray, secondary depth', 0),    
    
-- Neutral colors    
((SELECT id FROM brands WHERE slug = 'dropframe'), 'neutral.light', '#F5F5F5', '245, 245, 245', '3, 2, 2, 0', 'Cool Gray 1 C', 'Background surfaces, light base', 0),    
((SELECT id FROM brands WHERE slug = 'dropframe'), 'neutral.dark', '#212121', '33, 33, 33', '75, 68, 67, 70', 'Black 6 C', 'Primary text, high emphasis', 0),    
((SELECT id FROM brands WHERE slug = 'dropframe'), 'neutral.white', '#FFFFFF', '255, 255, 255', '0, 0, 0, 0', 'White', 'Pure white — cards, overlays', 0);
...
VALUES  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'services', 'brand-systems', 'icons/services/brand-systems.svg', '24x24', '1.5px', 'Brand Systems Architecture — modular grid', 0),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'services', 'creative-production', 'icons/services/creative-production.svg', '24x24', '1.5px', 'Creative Production — frame/filmstrip', 0),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'services', 'ecommerce-enable', 'icons/services/ecommerce-enable.svg', '24x24', '1.5px', 'Ecommerce Enablement — shopping cart with motion', 0),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'services', 'launch-campaigns', 'icons/services/launch-campaigns.svg', '24x24', '1.5px', 'Launch Campaigns — rocket/launch', 0),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'services', 'operational-consulting', 'icons/services/operational-consulting.svg', '24x24', '1.5px', 'Operational Consulting — checklist/process flow', 0);

-- Interface Icons  
INSERT INTO brand_icons (brand_id, category, name, file_path, size, stroke_width, description, is_primary_glyph)  
VALUES  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'interface', 'frame', 'icons/interface/frame.svg', '24x24', '1.5px', 'Frame — crop marks, filmstrip borders', 1),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'interface', 'grid', 'icons/interface/grid.svg', '24x24', '1.5px', 'Grid — modular layout', 0),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'interface', 'motion', 'icons/interface/motion.svg', '24x24', '1.5px', 'Motion — directional arrows, flow', 0),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'interface', 'scale', 'icons/interface/scale.svg', '24x24', '1.5px', 'Scale — size indicators, growth', 0),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'interface', 'precision', 'icons/interface/precision.svg', '24x24', '1.5px', 'Precision — target, alignment guides', 0);

-- Navigation Icons  
INSERT INTO brand_icons (brand_id, category, name, file_path, size, stroke_width, description, is_primary_glyph)  
VALUES  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'nav', 'services', 'icons/nav/services.svg', '24x24', '1.5px', 'Services navigation — grid icon', 0),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'nav', 'portfolio', 'icons/nav/portfolio.svg', '24x24', '1.5px', 'Portfolio navigation — frame/gallery', 0),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'nav', 'process', 'icons/nav/process.svg', '24x24', '1.5px', 'Process navigation — flow diagram', 0),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'nav', 'contact', 'icons/nav/contact.svg', '24x24', '1.5px', 'Contact navigation — message', 0),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'nav', 'menu', 'icons/nav/menu.svg', '24x24', '1.5px', 'Menu — horizontal lines', 0);

-- Status Icons  
INSERT INTO brand_icons (brand_id, category, name, file_path, size, stroke_width, description, is_primary_glyph)  
VALUES  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'status', 'in-progress', 'icons/status/in-progress.svg', '24x24', '1.5px', 'In progress — circular progress', 0),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'status', 'complete', 'icons/status/complete.svg', '24x24', '1.5px', 'Complete — checkmark in frame', 0),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'status', 'ready', 'icons/status/ready.svg', '24x24', '1.5px', 'Ready — green indicator', 0),  
((SELECT id FROM brands WHERE slug = 'dropframe'), 'status', 'pending', 'icons/status/pending.svg', '24x24', '1.5px', 'Pending — clock', 0);
...
Dropframe palette keeps backgrounds clean and modern without competing with accent colors.', 1),
